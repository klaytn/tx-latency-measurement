...
async function sendTx() {
  ...
  try {
    ...
    if (balance < parseFloat(process.env.BALANCE_ALERT_CONDITION_IN_NEAR)) {
      sendSlackMsg(
        `Current balance of <${process.env.SCOPE_URL}/accounts/${sender}|${sender}> is less than ${process.env.BALANCE_ALERT_CONDITION_IN_NEAR} NEAR! balance=${balance} NEAR`
      );
    }
    ...
    const block = await provider.sendJsonRpc("block", [accessKey.block_hash]);
    ...
    for (let i = 0; i < chunkList.length; i++) {
      const chunk = await provider.sendJsonRpc("chunk", [chunkList[i]]);
    ...
    data.txFee =
      (Number(result.transaction_outcome.outcome.tokens_burnt) +
        Number(result.receipts_outcome[0].outcome.tokens_burnt)) *
      10 ** -24;
    var NEARtoUSD;
