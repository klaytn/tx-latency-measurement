...
async function sendZeroSol() {
  ...
  try {
    ...
    if (balance * 10 ** -9 < parseFloat(process.env.BALANCE_ALERT_CONDITION_IN_SOL)) {
      sendSlackMsg(
        `Current balance of <${process.env.SCOPE_URL}/address/${keypair.publicKey}?cluster=${
          process.env.CLUSTER_NAME
        }|${keypair.publicKey}> is less than ${
          process.env.BALANCE_ALERT_CONDITION_IN_SOL
        } SOL! balance=${balance * 10 ** -9} SOL`
      );
    }
    ...
    await connection.getLatestBlockhashAndContext().then(async (result) => {
      ...
      await connection
        .getBlock(result.context.slot, { maxSupportedTransactionVersion: 0 })
        .then((response) => {
