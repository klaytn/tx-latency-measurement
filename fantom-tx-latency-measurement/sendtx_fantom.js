...
async function sendTx() {
  ...
  try {
    ...
    if (balance < parseFloat(process.env.BALANCE_ALERT_CONDITION_IN_FTM)) {
      sendSlackMsg(
        `Current balance of <${process.env.SCOPE_URL}/address/${signer.address}|${signer.address}> is less than ${process.env.BALANCE_ALERT_CONDITION_IN_FTM} FTM! balance=${balance} FTM`
      );
    }
    ...
    if (gasPrice * 10 ** -9 > process.env.MAX_GAS_PRICE) {
      console.log(
        `Gas Price ${gasPrice * 10 ** -9} Gwei is bigger than Gas Price limit ${
    ...
    var RLPEncodedTx;
    await web3.eth.accounts.signTransaction(rawTx, privateKey).then((result) => {
