...
async function sendTx() {
  ...
  try {
    ...
    if (balance < parseFloat(process.env.BALANCE_ALERT_CONDITION_IN_BNB)) {
      sendSlackMsg(
        `Current balance of <${process.env.SCOPE_URL}/address/${signer.address}|${signer.address}> is less than ${process.env.BALANCE_ALERT_CONDITION_IN_BNB} BNB! balance=${balance} BNB`
      );
    }
